...
type worker struct {
	debug  bool
	mode   int // send 0, recv 1, enc 2, dec 3, help 4
	pw     string
	msg    string
...
// print help text
func (tbox *worker) printhelp() {
	fmt.Println("YAS-cli v1.0 [2025 k-atusa]")
	fmt.Println("yas [mode] [options] targets[...]")
	fmt.Println("mode : -s, -r, -e, -d, -help (send receive encrypt decrypt help)")
	fmt.Println("options : -pw password, -msg message, -o output_path, -debug")
	fmt.Println("targets : file_path, dir_path, ip:port")
}

// manual config input
func (tbox *worker) getconfig() {
	fmt.Printf("interpreting mode enabled\nuse 'yas -help' to see help\n===== manual configuration =====\n")
