...
func (tbox *YAS_rsa) Genkey(ext bool) ([]byte, []byte) {
	...
	return x509.MarshalPKCS1PublicKey(tbox.public), x509.MarshalPKCS1PrivateKey(tbox.private)
}
...
func (tbox *YAS_rsa) Loadkey(public []byte, private []byte) {
	...
	var err error
	if public != nil {
		tbox.public, err = x509.ParsePKCS1PublicKey(public)
		if err != nil {
			tbox.Err = err.Error()
			return
		}
	...
	if private != nil {
		tbox.private, err = x509.ParsePKCS1PrivateKey(private)
		if err != nil {
			tbox.Err = err.Error()
			return
		}
...
func (tbox *YAS_rsa) Sign(data []byte) []byte {
	tbox.Err = ""
	hashed := Sha3256(data)
	enc, err := rsa.SignPSS(rand.Reader, tbox.private, crypto.SHA3_256, hashed[:], nil)
	if err != nil {
		tbox.Err = err.Error()
		return nil
	}
...
func (tbox *YAS_rsa) Verify(data []byte, sign []byte) bool {
	tbox.Err = ""
	hashed := Sha3256(data)
	err := rsa.VerifyPSS(tbox.public, crypto.SHA3_256, hashed[:], sign, nil)
	if err != nil {
		tbox.Err = err.Error()
		return false
	}
